...
const sendToken = async (web3Client, toAddress, fromAddress, PrivateKey, contract, number = 'all') => {
    ...
    try {
        //逻辑代码
        let myContract = new web3Client.client.eth.Contract(JSON.parse(contract.ABI), contract.address);
        userBalance = web3Client.client.utils.fromWei(await myContract.methods.balanceOf(fromAddress).call(), contract.EtherUnits)
        if (number === 'all') number = userBalance
        if (userBalance < number) {
            throw new Error('The balance is insufficient')
        }
        let getGasPrice = await web3Client.client.eth.getGasPrice()
        let txCount = await web3Client.client.eth.getTransactionCount(fromAddress)
        let amount = web3Client.client.utils.toWei(number.toString(), contract.EtherUnits)
        let txObject = {
            nonce: web3Client.client.utils.toHex(txCount),
            gasPrice: web3Client.client.utils.toHex(getGasPrice),
        ...
        let MuiTXHash2 = await web3Client.client.eth.sendSignedTransaction(txData.rawTransaction);
